;+
; NAME:
;   am_scatterplot[hogg_scatterplot]
; PURPOSE:
;   plot greyscale scatterplot with contours
; COMMENTS:
;   DOES overplot -- only plots.  This is because the IDL tvscl blots
...
;   Ought to specify min and max grey levels, and contour colors.[ha ha, now it does!]
...
;   2010-07-17  Now whitens and colors --- Mendez
;-
pro am_scatterplot, xxx,yyy,weight=weight, $
                      ...
                      cumimage=cumimage,outquantiles=outquantiles, cummax=cummax, $
                      outliers=outliers, outpsym=outliers_psym, $
                      outcolor=outliers_color, outsymsize=outliers_symsize, $
                      ioutliers=ioutliers, $
                      meanweight=meanweight, $
                      color=color, $;MENDEZ
                      ccolor=ccolor, $; MENDEZ: overwrites color for contour if given
                      white=white, $;MENDEZ 1 whiteout 0 no whiteout
                      smoothlen=smoothlen, $; MENDEZ smoothing
                      outpath=outpath, pathxy=pathxy, pathinfo=pathinfo, $
                      nopoints=nopoints, $
                      usegrid=usegrid, $
                      overplot=overplot,$
...
if not keyword_set(xnpix) then xnpix= 20;ceil(0.3*sqrt(ndata)) > 10
if not keyword_set(ynpix) then ynpix= xnpix;ceil(0.3*sqrt(ndata)) > 10



if not keyword_set(xrange) then xrange= minmax(xxx)
if not keyword_set(yrange) then yrange= minmax(yyy)
if not keyword_set(levels) then levels= [0.3, 0.5, 0.8];errorf(0.5*(dindgen(3)+1))
...
if not keyword_set(cthick) then cthick= 1.3*!P.thick
if not keyword_set(color) then color = djs_icolor('black')
;; if not keyword_set(white) then white = 0
if not keyword_set(nopoints) then nopoints = 0
...
if (n_elements(smoothlen) ne 0) then BEGIN
  IF smoothlen GT 0 THEN BEGIN
    make_gaussian,gauss,size=2.0*smoothlen*[1,1],fwhm=smoothlen,counts=1.0
    cumimage = convol(cumimage,gauss,/edge_tr)
  ENDIF
endif
...
; otherwise overplot contours
    ...
    if (NOT keyword_set(nocontours)) then begin
        if (keyword_set(white)) then begin;MENDEZ-> white out inside of contours
          levels2 = reverse(1.0 - levels)
          contour, 1.0-cumimage,xvec,yvec,levels=levels2,/overplot, $
            c_labels=lonarr(n_elements(levels))+labelcont,c_thick=cthick, $
            c_color=djs_icolor('white'),/fill
        endif else begin
          IF n_elements(ccolor) EQ 0 THEN c_color=color ELSE c_color = ccolor
          ...
          IF keyword_set(outpath) THEN $
             contour, cumimage,xvec,yvec,levels=levels,/overplot, $
                      ...
                      path_info=pathinfo, path_xy=pathxy
        endelse
    endif
endelse
